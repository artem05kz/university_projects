...
void fill(MyStruct& s, int temp)
{
    ...
    if (temp == 0)
    {
        for (typesize i = 0; i < s.size; i++)
        {
            ...
            cin >> temp1;
            s.A.emplace_back(temp1);
        }
    }
    else if (temp == 1)
    {
        for (typesize i = 0; i < s.size; i++)
            s.A.emplace_back(rand());
...
int deletePos(MyStruct& s, typesize pos)
{
    if (pos <= s.size && pos >= 0)
    {
        auto iter = s.A.cbegin(); // константный итератор указывает на первый элемент
        s.A.erase(iter + pos);
        s.size = s.A.size();
        return 0;
    }
